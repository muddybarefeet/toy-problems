...
RangeClass.prototype.includes = function (target) {
  //return whether or not the range includes the passed value
  if (this.start === this.end) console.log("There is only one item in the range");
  if (this.step < 0) {
    for (var i = this.start; i >= this.end; i-=Math.abs(this.step)) {
      if (i === target) {
        return true;
