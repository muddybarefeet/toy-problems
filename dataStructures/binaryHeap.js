...
BinaryHeap.prototype.insert = function (value) {
  ...
  var currentIndex = this._heap.length-1;
  var parentIndex = Math.floor((currentIndex-1)/2);

  while (this._heap[parentIndex] > this._heap[currentIndex]) {
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  //swap the first and last things in heap
  //remove last thing
  ...
  //then move new root back to correct position in the array
  var currentIndex = 0;
  var children = [(currentIndex*2)+1, (currentIndex*2)+2];

  while (this._heap[currentIndex] > this._heap[children[0]] || this._heap[currentIndex] > this._heap[children[1]]) {
    var temp = this._heap[currentIndex];
    //edit should really swap the parent with the smaller child - to be updated
    if ( (this._heap[currentIndex] > this._heap[children[0]] && this._heap[currentIndex] > this._heap[children[1]]) || (this._heap[currentIndex] > this._heap[children[0]] && this._heap[currentIndex] < this._heap[children[1]]) ) {
      //swap with first
      this._heap[currentIndex] = this._heap[children[0]];
      this._heap[children[0]] = temp;
      currentIndex = children[0];
    }

    if (this._heap[currentIndex] < this._heap[children[0]] && this._heap[currentIndex] > this._heap[children[1]]) {
      //swap with  the second
      this._heap[currentIndex] = this._heap[children[1]];
      this._heap[children[0]] = temp;
      currentIndex = children[1];
    }

    children = [(currentIndex*2)+1, (currentIndex*2)+2];
  }
  return root;
